function[fitscore] = GroupConnParcellation(subject_list, session, datadir, mmpdir, outdir, correction)
    ...
    mask = importdata(fullfile(mmpdir, 'medialwallmask.1D');
    mask = mask';
    mask = logical(mask);
    ...
    for i = 1:length(areas)
        ...
        for h = 1:length(subject_list)
            try
                conn_L = importdata(fullfile(datadir, sprintf('%s_AuditoryParcellation_%s_meanconn_%s_L.1D', subject_list(h), session, area);
            ...
            try
                conn_R = importdata(fullfile(datadir, sprintf('%s_AuditoryParcellation_%s_meanconn_%s_R.1D', subject_list(h), session, area);
        ...
        for j= 1:2
            ...
            filename = fullfile(outdir, sprintf('GroupConn_%s_%s_%s', session, area, correction);
        ...
        colorbar('visible', 'off');
        filename = fullfile( outdir , sprintf('GroupConn_contrast_%s_%s_%s', session, area, correction);
        saveas(gcf, [filename '.png']); close all;
        fid = fopen([filename '.1D'],'w');
        fprintf(fid, '%u\n', t_thresh);
        fclose(fid);
